...
async def on_message(message):
    ...
    elif hellion_role in message.author.roles:
    ...
    #HOW TO GET STUCK
        ...
        for i in range(0, len(limbo_list)):
            if limbo_list[i] in message.content.lower():
                await bot_garden.send("**{}** should be confined to limbo. **current roles:** {}.".format(message.author.name, roles_list(message.author)))
                break

        #lust - "sin bin"
        if "sin bin" in message.content.lower():
            await bot_garden.send("**{}** should be confined to lust. **current roles:** {}.".format(message.author.name, roles_list(message.author)))

        #anger - cursing
        curse_list = ["fuck", "shit", "bitch", "binch", "bxtch", "dick", "crap", "ass"]
        for i in range(0, len(curse_list)):
            if curse_list[i] in message.content.lower():
                await bot_garden.send("**{}** should be confined to anger. **current roles:** {}.".format(message.author.name, roles_list(message.author)))
                break
        
        #heresy - using "god" or "hell"
        if "god" in message.content.lower() or "hell " in message.content.lower() or "damn" in message.content.lower():
            await bot_garden.send("**{}** should be confined to heresy. **current roles:** {}.".format(message.author.name, roles_list(message.author)))

        #fraud - "honest", "tbh"
        if "honest" in message.content.lower() or "tbh" in message.content.lower():
            await bot_garden.send("**{}** should be confined to fraud. **current roles:** {}.".format(message.author.name, roles_list(message.author)))

        #treachery - "shame"
        if "sham" in message.content.lower():
            await bot_garden.send("**{}** should be confined to treachery. **current roles:** {}.".format(message.author.name, roles_list(message.author)))
...
async def on_reaction_add(reaction, user):
    ...
    #violence stuckitude - emoji reacts outside of violence 
        ...
        await bot_garden.send("**{}** should be confined to violence. **current roles:** {}.".format(user.name, roles_list(user)))
